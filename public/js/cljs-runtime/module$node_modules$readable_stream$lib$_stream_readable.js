shadow$provide.module$node_modules$readable_stream$lib$_stream_readable=function(global,process,require,module,exports,shadow$shims){function prependListener(emitter,event,fn){if("function"===typeof emitter.prependListener)return emitter.prependListener(event,fn);if(emitter._events&&emitter._events[event])isArray(emitter._events[event])?emitter._events[event].unshift(fn):emitter._events[event]=[fn,emitter._events[event]];else emitter.on(event,fn)}function ReadableState(options,stream){Duplex=Duplex||
require("module$node_modules$readable_stream$lib$_stream_duplex");options=options||{};stream=stream instanceof Duplex;this.objectMode=!!options.objectMode;stream&&(this.objectMode=this.objectMode||!!options.readableObjectMode);var hwm=options.highWaterMark,readableHwm=options.readableHighWaterMark,defaultHwm=this.objectMode?16:16384;this.highWaterMark=hwm||0===hwm?hwm:stream&&(readableHwm||0===readableHwm)?readableHwm:defaultHwm;this.highWaterMark=Math.floor(this.highWaterMark);this.buffer=new BufferList;
...
this.encoding=options.encoding)}function Readable(options){Duplex=Duplex||require("module$node_modules$readable_stream$lib$_stream_duplex");if(!(this instanceof Readable))return new Readable(options);this._readableState=new ReadableState(options,this);this.readable=!0;options&&("function"===typeof options.read&&(this._read=options.read),"function"===typeof options.destroy&&(this._destroy=options.destroy));Stream.call(this)}function readableAddChunk(stream,chunk,encoding,addToFront,skipChunkCheck){var state=
stream._readableState;if(null===chunk)state.reading=!1,state.ended||(state.decoder&&(chunk=state.decoder.end())&&chunk.length&&(state.buffer.push(chunk),state.length+=state.objectMode?1:chunk.length),state.ended=!0,emitReadable(stream));else{if(!skipChunkCheck){skipChunkCheck=chunk;var er;Buffer.isBuffer(skipChunkCheck)||skipChunkCheck instanceof OurUint8Array||"string"===typeof skipChunkCheck||void 0===skipChunkCheck||state.objectMode||(er=new TypeError("Invalid non-string/buffer chunk"));var er$jscomp$0=
er}er$jscomp$0?stream.emit("error",er$jscomp$0):state.objectMode||chunk&&0<chunk.length?("string"===typeof chunk||state.objectMode||Object.getPrototypeOf(chunk)===Buffer.prototype||(chunk=Buffer.from(chunk)),addToFront?state.endEmitted?stream.emit("error",Error("stream.unshift() after end event")):addChunk(stream,state,chunk,!0):state.ended?stream.emit("error",Error("stream.push() after EOF")):(state.reading=!1,state.decoder&&!encoding?(chunk=state.decoder.write(chunk),state.objectMode||0!==chunk.length?
addChunk(stream,state,chunk,!1):state.readingMore||(state.readingMore=!0,pna.nextTick(maybeReadMore_,stream,state))):addChunk(stream,state,chunk,!1))):addToFront||(state.reading=!1)}return!state.ended&&(state.needReadable||state.length<state.highWaterMark||0===state.length)}function addChunk(stream,state,chunk,addToFront){state.flowing&&0===state.length&&!state.sync?(stream.emit("data",chunk),stream.read(0)):(state.length+=state.objectMode?1:chunk.length,addToFront?state.buffer.unshift(chunk):state.buffer.push(chunk),
